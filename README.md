...
```bash
chap evaluate --model-name /path/to/temp_r_model --dataset-csv your_data.csv --report-filename report.pdf
```

Or with a built-in dataset:

```bash
chap evaluate --model-name /path/to/temp_r_model --dataset-name ISIMIP_dengue_harmonized --dataset-country brazil --report-filename report.pdf
```
